...
| — | マーカーのタイル方式（大量マーカー） | 済み |
| — | サンプルアプリへの組み込み | 済み |

### 空タイルの 404 に注意

マーカーのタイルを配るローカルサーバは、**マーカーが 1 つも無いタイルに 404 を返す**。
MapLibre などはそれを空タイルとして扱うが、この SDK は 404 を**エラー**として扱い、
そのタイルを「存在しない」と記録する。子タイルが無い領域はステンシルマスクに穴が開き、
保持されている粗い親タイルがそこだけ透けて残る。

`OpenMobileMapsTileLoader` がローカルサーバからのエラーだけを透明タイルへ読み替えて
いる。**素の `MCTextureLoader` を渡さないこと。**
